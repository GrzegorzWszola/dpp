...
from .game import GameDTO, GameUI, PlayerDTO, MoveDTO
...
class MyWindow(QMainWindow):
    ...
    def on_start(self):
        ...
        player_1 = PlayerDTO(id=1, name=player_1_name, is_ai=False)
        if self.ui.witchComputerCheckbox.isChecked():
            player_2 = PlayerDTO(id=2, name=player_2_name, is_ai=True)
        else:
            player_2 = PlayerDTO(id=2, name=player_2_name, is_ai=False)
        ...
        loaded_game_state = {
            "player_1": asdict(player_1),
            "player_2": asdict(player_2),
            "size":     selected_size,
        ...
        self.game = GameDTO(
            ...
            current_player=PlayerDTO(**response["current_player"]),
            winner=PlayerDTO(**response["winner"]) if response["winner"] else None,
            move=MoveDTO(**response["move"]) if response["move"] else None,
            move_list=[MoveDTO(**m) for m in response["move_list"]]
        )
    ...
    def load_state(self, path: str) -> list[MoveDTO]:
        self.ui.label_input_path.setText(os.path.basename(path))
        with open(path) as f:
            data = json.load(f)

        size = data["size"]
        moves = [MoveDTO(player=m["player"], row=m["row"], col=m["col"]) for m in data["moves"]]
        return size, moves
    ...
    def update_game_state(self, result_dict):
        ...
        self.game = GameDTO(
            ...
            current_player=PlayerDTO(**result_dict["current_player"]),
            winner=PlayerDTO(**result_dict["winner"]) if result_dict["winner"] else None,
            move=MoveDTO(**result_dict["move"]) if result_dict["move"] else None,
            move_list=[]
        )
