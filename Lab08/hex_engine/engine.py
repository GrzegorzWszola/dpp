...
class Engine:
    ...
    def move(self, row=None, col=None):
        ...
        return {
            ...
            "current_player": self.current_player.player_to_dict(),
            "winner": winner.player_to_dict() if winner else None,
            "move": asdict(current_move)
        }
    ...
    def start_game(self):
        ...
        return {
            "matrix": self.board.game_state_matrix.tolist(),
            "current_player": self.current_player.player_to_dict(),
            "winner": self.current_player.player_to_dict() if winner else None,
            "move": asdict(self.board.move_list[-1]) if self.board.move_list else None,
            "move_list": [asdict(move) for move in self.board.move_list] if self.board.move_list else []
        }